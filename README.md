...
| 物理启发的体积极光：程序化薄帘、高度发射分层、自适应视线积分 |
| --- |
| ![4K volumetric aurora over procedural mountains](docs/assets/aurora_4k.png) |
...
- **实时体积极光**：根据电子通量足迹与高度沉积相乘的思路构造四层程序化薄帘，使用绿氧、红氧和低空氮发射分层、自适应视线积分、四分之一分辨率 `RGBA16F` 缓冲及全分辨率深度合成；完整推导见[极光渲染说明](docs/AURORA_RENDERING.md)。
...
| Aurora Volume | 动画、采样预算、发光强度、帘幕高度/距离/间距/厚度/褶皱/扰动，以及三组发射颜色与权重 |
...
```mermaid
flowchart LR
    FOOTPRINT["Procedural curtain footprint"] --> VOLUME["Emissive volume"]
...
绿、红和蓝紫发射层分别近似不同高度的氧与氮发射；薄帘通量场保持沿高度连续，避免退化成普通噪声烟雾。实现明确区分实时视觉近似与科学模拟，公式、限制和参考资料见[实时极光体积渲染](docs/AURORA_RENDERING.md)。
...
- 基于极光电子通量足迹与高度沉积分解实现程序化发光薄帘、自适应视线积分和微弱体积透射；在四分之一分辨率 HDR 缓冲完成体积计算，以全分辨率深度遮罩合成，并接入 Bloom、ACES 与 ImGui 参数调试。
...
- Volumetric Aurora 已完成 `1600 x 900` ImGui 交互检查与 `3840 x 2160` Back Buffer 导出；控制面板截帧显示约 `2.17 ms`，体积缓冲、深度遮挡、Bloom 和 ACES 均正常。
