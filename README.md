...
| --- |
| [![4K volumetric aurora over procedural mountains](docs/assets/aurora_4k.png)](https://raw.githubusercontent.com/lizuoshuo-lab/OpenGL/main/docs/assets/aurora-preview-1080p.mp4) |
...
- **实时体积极光**：根据电子通量足迹与高度沉积相乘的思路构造三层随机带状薄帘。低频 fBm 差值形成断续区域，独立方向、宽度、亮边和次级褶皱打破平行重复，再接入绿氧、红氧和低空氮发射分层、自适应视线积分、四分之一分辨率 `RGBA16F` 缓冲及全分辨率深度合成；完整推导见[极光渲染说明](docs/AURORA_RENDERING.md)。
...
绿、红和蓝紫发射层分别近似不同高度的氧与氮发射。三层薄帘各自具有不同走向，噪声差与路径扰动只改变带形、断续和宽度，不把主体退化成均匀烟雾或重复竖纹。实现明确区分实时视觉近似与科学模拟，公式、限制和参考资料见[实时极光体积渲染](docs/AURORA_RENDERING.md)。
...
- 基于极光电子通量足迹与高度沉积分解实现三层随机带状发光薄帘。通过域扭曲 fBm 差值、独立路径方向、非对称亮边和次级褶皱消除规则竖纹，再执行自适应视线积分和微弱体积透射；在四分之一分辨率 HDR 缓冲完成体积计算，以全分辨率深度遮罩合成，并接入 Bloom、ACES 与 ImGui 参数调试。
...
- Volumetric Aurora 已完成 `1600 x 900` ImGui 交互检查与 `3840 x 2160` 全屏导出；控制面板截帧显示约 `2.01 ms`，随机带形、体积缓冲、深度遮挡、Bloom 和 ACES 均正常。
...
- 已生成 12 秒、3840 x 2160、30 FPS、360 帧的 H.264 极光实机视频，并通过 `ffprobe` 自动校验；同时提供 GitHub 可直接播放的 1080p 预览。
