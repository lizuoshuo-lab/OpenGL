...
| 物理启发的体积极光：随机带状薄帘、高度发射分层、自适应视线积分 |
| --- |
| [![Animated isolated aurora ribbons over procedural mountains](docs/assets/aurora-preview.gif)](https://raw.githubusercontent.com/lizuoshuo-lab/OpenGL/main/docs/assets/aurora-preview-1080p.mp4) |
...
- **实时体积极光**：根据电子通量足迹与高度沉积相乘的思路构造三层随机带状薄帘。低频 fBm 差值形成断续区域，独立方向、宽度、亮边和次级褶皱打破平行重复；低通量软阈值抑制掠射角积累，避免整片天空染色。随后接入绿氧、红氧和低空氮发射分层、自适应视线积分、四分之一分辨率 `RGBA16F` 缓冲及全分辨率深度合成；完整推导见[极光渲染说明](docs/AURORA_RENDERING.md)。
...
| Aurora Volume | 动画、采样预算、发光强度、帘幕高度/距离/间距/厚度/褶皱/扰动、带状变化、射线细节，以及三组发射颜色与权重 |
...
绿、红和蓝紫发射层分别近似不同高度的氧与氮发射。三层薄帘各自具有不同走向，噪声差与路径扰动只改变带形、断续和宽度；低密度样本经过软阈值后不会沿掠射视线积累成全屏色雾。实现明确区分实时视觉近似与科学模拟，公式、限制和参考资料见[实时极光体积渲染](docs/AURORA_RENDERING.md)。
...
- 基于极光电子通量足迹与高度沉积分解实现三层随机带状发光薄帘。通过域扭曲 fBm 差值、独立路径方向、非对称亮边和次级褶皱消除规则竖纹，并使用低通量软阈值和远层衰减阻止掠射角铺色，再执行自适应视线积分和微弱体积透射；在四分之一分辨率 HDR 缓冲完成体积计算，以全分辨率深度遮罩合成，并接入 Bloom、ACES 与 ImGui 参数调试。
...
- Volumetric Aurora 已完成 `1600 x 900` ImGui 交互检查与 `3840 x 2160` 全屏导出；控制面板截帧显示约 `2.3 ms`，随机带形、低通量隔离、体积缓冲、深度遮挡、Bloom 和 ACES 均正常。
...
- 已生成 12 秒、3840 x 2160、30 FPS、360 帧的 H.264 极光实机视频，并通过 `ffprobe` 自动校验；同时提供 1080p 视频和 `960 x 540`、12 FPS、144 帧的 README 循环 GIF。
