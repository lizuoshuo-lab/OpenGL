...
| 地质行星与 GPU 小行星带：5000 实例、自转公转、视锥裁剪、实例化与平滑 LOD |
| --- |
| [![Animated geological planet and GPU asteroid belt](docs/assets/planet-showcase-preview.gif)](https://raw.githubusercontent.com/lizuoshuo-lab/OpenGL/main/docs/assets/opengl-ibl-preview-1080p.mp4) |

| 物理启发的体积极光：随机带状薄帘、全天空低频帘幕、高度发射分层、自适应视线积分 |
| --- |
| [![Animated full-sky aurora curtains over procedural mountains](docs/assets/aurora-preview.gif)](https://raw.githubusercontent.com/lizuoshuo-lab/OpenGL/main/docs/assets/aurora-preview-1080p.mp4) |

| 极光参数、渲染管线与性能面板 |
| --- |
| ![Volumetric aurora ImGui controls](docs/assets/aurora_controls.png) |
...
- **实时体积极光**：根据电子通量足迹与高度沉积相乘的思路构造三层随机带状薄帘。低频 fBm 差值形成断续区域，独立方向、宽度、亮边和次级褶皱打破平行重复；低通量软阈值只抑制无结构的掠射角 halo，独立的天空空间低频场则有意为整片星空加入带暗缝的青绿色漫射帘幕。所有噪声坐标都由连续时间推进，每一帧平滑形变。随后接入绿氧、红氧和低空氮发射分层、自适应视线积分、四分之一分辨率 `RGBA16F` 缓冲及全分辨率深度合成；完整推导见[极光渲染说明](docs/AURORA_RENDERING.md)。
...
| 分组 | 可操作内容 |
| --- | --- |
| Showcase | 十一个场景切换、30 秒自动演示、自动旋转、速度、角度、缩放、视角重置 |
| Aurora Volume | 动画、连续运动速度、采样预算、发光强度、全天空漫射强度、帘幕高度/距离/间距/厚度/褶皱/扰动、带状变化、射线细节，以及三组发射颜色与权重 |
...
键盘 `1` 到 `9` 可快速切换材质矩阵、六个模型、GPU Asteroid Belt 和 SSAO Gallery，`0` 切换到 Skeletal Animation，`F9` 切换到 Volumetric Aurora。`F10` 可重新开始或停止 30 秒自动演示，`F11` 可在全屏与窗口模式之间切换。相机支持鼠标观察及 `W/A/S/D/Q/E` 六方向移动；ImGui 捕获输入时不会误触相机。
...
flowchart LR
    ...
    LIGHTING --> AURORA["Quarter-res Aurora Volume"]
    DEPTH["Full-res GBuffer Depth"] --> COMPOSITE["Depth-aware HDR Composite"]
    AURORA --> COMPOSITE
    TRANSPARENT --> COMPOSITE
    COMPOSITE --> BLOOM["Bloom Pass"]
...
```mermaid
flowchart LR
    FOOTPRINT["Domain-warped random band field"] --> VOLUME["Emissive volume"]
    HEIGHT["Height deposition profiles"] --> VOLUME
    VOLUME --> MARCH["Adaptive ray integration"]
    MARCH --> HDR["Quarter-res RGBA16F"]
    DEPTH["Full-res depth"] --> UPSAMPLE["Masked upsample"]
    HDR --> UPSAMPLE
    UPSAMPLE --> POST["Additive composite + Bloom + ACES"]
```

绿、红和蓝紫发射层分别近似不同高度的氧与氮发射。三层薄帘各自具有不同走向，噪声差与路径扰动改变带形、断续和宽度；低密度样本经过软阈值后不会沿掠射视线产生失控 halo。独立的球面天空坐标低频场负责青绿色全天空漫射，通过域扭曲宽帘、纵向垂幕和大尺度暗区保持层次。实现明确区分实时视觉近似与科学模拟，公式、限制和参考资料见[实时极光体积渲染](docs/AURORA_RENDERING.md)。
...
- 极光薄帘通量、高度发射与自适应积分：[assets/shaders/aurora.frag](assets/shaders/aurora.frag)、[glframework/material/auroraMaterial.h](glframework/material/auroraMaterial.h)
...
导出体积极光 4K 画面；去掉 `--no-ui` 可同时保留完整控制面板：

```powershell
.\Debug\openglStudy.exe --fullscreen --showcase=11 --no-ui --screenshot=verification\aurora_4k.png
```

自动录制并校验 12 秒、4K、30 FPS 的极光视频：

```powershell
.\tools\record_showcase_4k.ps1 -BuildDirectory .\build -Profile Aurora -FfmpegPath ffmpeg
```
...
- 基于极光电子通量足迹与高度沉积分解实现三层随机带状发光薄帘。通过域扭曲 fBm 差值、独立路径方向、非对称亮边和次级褶皱消除规则竖纹，并使用低通量软阈值和远层衰减隔离主体亮带；额外构造球面天空坐标低频漫射场，使整片星空具有连续变化的青绿色帘幕与大尺度暗缝。体积计算写入四分之一分辨率 HDR 缓冲，以全分辨率深度遮罩合成，并接入 Bloom、ACES 与 ImGui 参数调试。
...
- Volumetric Aurora 已完成 `1600 x 900` ImGui 交互检查与 `3840 x 2160` 全屏导出；控制面板截帧显示约 `2.2 ms`，随机亮带、全天空低频帘幕、连续逐帧形变、体积缓冲、深度遮挡、Bloom 和 ACES 均正常。
...
- 已生成 12 秒、3840 x 2160、30 FPS、360 帧的 H.264 极光实机视频，并通过 `ffprobe` 自动校验；同时提供 1080p 视频和 `768 x 432`、12 FPS、144 帧的 README 循环 GIF。
