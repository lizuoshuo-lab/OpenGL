...
- **动态小行星带**：5000 颗、三套真实月岩模型组成的高密度小行星带，支持中心地质行星自转、小行星独立自转与整条带状轨道公转，并可一键对比逐对象绘制与 Frustum Culling + GPU Instancing + 三级 LOD。
...
| 分组 | 可操作内容 |
| --- | --- |
| Showcase | 九个场景切换、30 秒自动演示、自动旋转、速度、角度、缩放、视角重置 |
...
| IBL | 12 个输出视图、环境强度、Reflection LOD、宇宙底色与星点强度 |
...
键盘 `1` 到 `9` 可快速切换材质矩阵、六个模型、GPU Asteroid Belt 和 SSAO Gallery，`F10` 可重新开始或停止 30 秒自动演示，`F11` 可在全屏与窗口模式之间切换。相机支持鼠标观察及 `W/A/S/D/Q/E` 六方向移动；ImGui 捕获输入时不会误触相机。
...
| Deep-space starfield | 天空盒纯暗宇宙底色与随机微小星点 | GLSL procedural | Project code |
...
- 使用 AABB Frustum Culling、GPU Instancing 和三级 Mesh LOD 优化 5000 颗多材质小行星；Debug 1600 x 900 实测 Draw Call 从 10233 降至 28。
- 为地质行星、小行星实例和轨道父节点实现分层动画，运动参数全部在 ImGui 中可控；基准运行时自动冻结动画，保证 A/B 采样可重复。
- 实现绕中心行星的自动环绕相机，并让相机始终朝向地质行星；天空盒使用纯暗宇宙底色和随机冷暖色微小星点，不依赖银河照片或高亮天体。
...
- 自动 A/B 基准通过，测试结束后进程可自行关闭；CPU `381.087 -> 25.385 ms`、GPU `179.753 -> 0.670 ms`、Draw Call `10233 -> 28`、三角形 `49.026M -> 4.170M`。
