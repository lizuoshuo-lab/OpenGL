...
![Deferred PBR asteroid belt rendered at 4K](docs/assets/showcase_p0_asteroid_belt_4k.png)
...
- **场景优化 A/B**：2500 颗、三套真实月岩模型组成的小行星带，可一键对比逐对象绘制与 Frustum Culling + GPU Instancing + 三级 LOD。
...
| IBL | 12 个输出视图、环境强度、Reflection LOD、背景黑位与程序化星点 |
| Material | Metallic、Roughness、AO、Normal 强度、实例表面变化；三组小行星材质可分别调色 |
...
- 使用 AABB Frustum Culling、GPU Instancing 和三级 Mesh LOD 优化 2500 颗多材质小行星；Debug 1600 x 900 实测 Draw Call 从 4958 降至 28。
...
- 自动 A/B 基准通过，测试结束后进程可自行关闭；CPU `175.311 -> 12.249 ms`、GPU `65.955 -> 0.413 ms`、Draw Call `4958 -> 28`、三角形 `23.860M -> 2.078M`。
