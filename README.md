...
![Animated geological planet and GPU asteroid belt rendered at 4K](docs/assets/showcase_p0_asteroid_belt_4k.png)
...
- **动态小行星带**：2500 颗、三套真实月岩模型组成的小行星带，支持中心地质行星自转、小行星独立自转与整条带状轨道公转，并可一键对比逐对象绘制与 Frustum Culling + GPU Instancing + 三级 LOD。
...
| Orbital Motion | 运动总开关、小行星带公转速度、行星自转速度、小行星自转倍率、运动重置 |
...
| IBL | 12 个输出视图、环境强度、Reflection LOD、ESO 全天银河背景亮度与黑位 |
...
键盘 `1` 到 `8` 可快速切换材质矩阵、六个模型和 GPU Asteroid Belt，`F10` 可重新开始或停止 30 秒自动演示，`F11` 可在全屏与窗口模式之间切换。相机支持鼠标观察及 `W/A/S/D/Q/E` 六方向移动；ImGui 捕获输入时不会误触相机。
...
| The Milky Way panorama | 6000 x 3000 可见天空背景 | [ESO / S. Brunier](https://www.eso.org/public/images/eso0932a/) | [CC BY 4.0](https://www.eso.org/public/outreach/copyright/) |
...
纹理处理和署名细节见 [assets/textures/README.md](assets/textures/README.md)。可见银河照片与 IBL 光照输入彼此独立，便于分别控制画面构图和 PBR 能量。
...
- 使用 AABB Frustum Culling、GPU Instancing 和三级 Mesh LOD 优化 2500 颗多材质小行星；Debug 1600 x 900 实测 Draw Call 从 5194 降至 27。
...
- 自动 A/B 基准通过，测试结束后进程可自行关闭；CPU `216.240 -> 13.523 ms`、GPU `83.178 -> 0.463 ms`、Draw Call `5194 -> 27`、三角形 `24.785M -> 1.960M`。
