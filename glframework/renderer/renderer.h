...
public:
	...
	void resetDrawStats() { mDrawStats = {}; }
	const RendererDrawStats& getDrawStats() const { return mDrawStats; }
...
public:
	...
	bool mSoftShadowEnabled{ false };
	Texture* mSoftShadowMap{ nullptr };
	glm::mat4 mSoftShadowMatrix{ 1.0f };
	glm::vec3 mSoftShadowLightPosition{ 0.0f, 8.0f, 0.0f };
	float mSoftShadowStrength{ 0.75f };
	float mSoftShadowBias{ 0.0015f };
	float mSoftShadowRadius{ 3.0f };

private:
	void projectObject(Object* obj);
	void renderDepthObject(Object* object);
	void recordDraw(const Geometry* geometry, std::uint64_t instanceCount = 1);
